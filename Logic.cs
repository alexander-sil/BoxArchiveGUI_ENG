...
namespace BoxArchiveGUI
{
    public class Logic
    {
        ...
        public static byte[][] PrepareData(FileInfo[] files)
        {
            ...
            if ((files.Length % 2) == 0)
            {
                ...
                for (int i = 0; i < (files.Length / 2); i++)
                {
                    ...
                    header.AddRange(Encoding.Unicode.GetBytes($"{0} " + name0 + "\t").Concat(Encoding.Unicode.GetBytes($"{1} " + name1 + "\t")).Cast<byte>());
                    ...
                    string debug1 = Encoding.Unicode.GetString(dataToPrepare.ToArray());
            ...
            else if (files.Length == 1)
            {
                ...
                finalHeader.AddRange(Encoding.Unicode.GetBytes($"{0} " + name0 + "\t").Cast<byte>());
                ...
                string debug1 = Encoding.Unicode.GetString(finalDataToPrepare.ToArray());
            ...
            else
            {
                ...
                finalHeader.AddRange(Encoding.Unicode.GetBytes($"{0} " + files[files.Length - 1].Name + "\t").Cast<byte>());
        ...
        public static void UnpackFiles(byte[] data, DirectoryInfo dir)
        {
            ...
            string toParse = Encoding.Unicode.GetString(data);
            ...
            for (int i = 0; i < parsed2.Length; i++)
            {
                ...
                parsedData[i] = Encoding.Unicode.GetBytes(parsed2[i]);
            }
